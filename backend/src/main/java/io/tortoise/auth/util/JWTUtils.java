...
public class JWTUtils {
    ...
    public static TokenInfo tokenInfoByToken(String token) {
        ...
        if (StringUtils.isEmpty(username) || ObjectUtils.isEmpty(userId) ){
            TortoiseException.throwException("token格式错误！");
        }
    ...
    public static boolean loginExpire(String token){
        if (Login_Interval==0) {
            // 默认超时时间是8h
            int minute = CommonBeanFactory.getBean(Environment.class).getProperty("tortoise.login_timeout", Integer.class, 8*60);
            // 分钟换算成毫秒
